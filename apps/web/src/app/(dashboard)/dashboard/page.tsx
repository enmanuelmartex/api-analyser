...
export default function DashboardPage() {
  ...
  const { data: issueStats } = useQuery<IssueStats>({ queryKey: ['issues', 'stats'], queryFn: () => issuesApi.stats(), refetchInterval: 30000 });
  ...
  return <PageContainer className="space-y-5 pb-10">
    ...
    <section aria-label="Security metrics" className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <MetricCard label="Scans" value={String(stats?.totalAssessments ?? 0)} icon={<IconActivity className="h-4 w-4" />} tone="muted" description="Completed security scans" />
    </section>
