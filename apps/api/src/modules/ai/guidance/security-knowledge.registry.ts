...
/**
 * Runtime security knowledge, versioned and retrievable.
 *
 * IMPORTANT: the skills available to a coding agent are NOT available to this
 * deployed application. Whatever a developer's tooling knows about OWASP or
 * cloud gateways exists only at development time. For the running product to
 * give grounded advice, that knowledge has to live here, in the application, in
 * a form it can retrieve and cite.
 *
 * The registry is deliberately data, not prose in a prompt string:
 *
 *  - It is versioned (`KNOWLEDGE_PACK_VERSION`), so guidance can be attributed
 *    to the knowledge that produced it and reproduced later.
 *  - It is retrieved selectively. Sending every playbook with every request
 *    would blow the context window and dilute the relevant material; only
 *    playbooks matching the finding's OWASP category, rule or detected
 *    technology are included.
 *  - Each entry carries references, so the model is asked to ground its answer
 *    in cited material rather than recall.
 */

export const KNOWLEDGE_PACK_VERSION = 'knowledge-2026.08.1';
...
export interface SecurityPlaybook {
  id: string;
  scope: PlaybookScope;
  /** Match keys, lowercased: OWASP ids, rule ids, technology names. */
  keys: string[];
  title: string;
  /** The substance handed to the model. Concise on purpose. */
  content: string;
...
const PLAYBOOKS: SecurityPlaybook[] = [
  // ── OWASP categories ──────────────────────────────────────────────────────
  {
    ...
    content:
      'Every request that names an object must re-check that the authenticated principal may act on THAT object. ' +
  ...
  },
  {
    ...
    content:
      'Validate token signature, issuer, audience and expiry on every request, and reject anything malformed with 401. ' +
  ...
  },
  {
    ...
    content:
      'Serialise responses from an explicit allowlist of properties rather than returning the persistence model. ' +
  ...
  },
  {
    ...
    content:
      'Apply rate limits per authenticated principal and per IP, at the edge and in the application. Bound every ' +
  ...
  },
  {
    ...
    content:
      'Deny by default and require an explicit role or permission for every route, especially administrative ones. ' +
  ...
  },
  {
    ...
    content:
      'The control has to sit on the business action, not on the endpoint. Rate-limit per authenticated principal, ' +
  ...
  },
  {
    ...
    content:
      'Validate any caller-supplied URL against an allowlist of hosts and schemes before fetching it. Resolve DNS first ' +
  ...
  },
  {
    ...
    content:
      'Send HSTS on HTTPS origins, `X-Content-Type-Options: nosniff`, a restrictive `Content-Security-Policy` for any ' +
  ...
  },
  {
    ...
    content:
      'Generate the inventory of deployed versions and hosts from deployment configuration rather than maintaining ' +
  ...
  },
  {
    ...
    content:
      'Treat every response from a third party as untrusted input: validate it against a schema before it reaches ' +
  ...
  // ── Frameworks ────────────────────────────────────────────────────────────
  {
    ...
    title: 'Express',
    content:
      'Use `helmet()` for security headers and configure `cors()` with an explicit origin list rather than `true`. ' +
  ...
  },
  {
    ...
    title: 'NestJS',
    content:
      'Register a global `ValidationPipe` with `whitelist` and `forbidNonWhitelisted` so unknown properties are ' +
  ...
  },
  {
    ...
    title: 'Django / DRF',
    content:
      'Set `DEFAULT_PERMISSION_CLASSES` to a deny-by-default value such as `IsAuthenticated`, and add object-level ' +
  ...
  },
  {
    ...
    title: 'Spring Boot',
    content:
      'Configure `SecurityFilterChain` to `authenticated()` by default with explicit `permitAll()` exceptions. Use ' +
  ...
  },
  {
    ...
    title: 'FastAPI',
    content:
      'Declare dependencies (`Depends`) for authentication on the router so routes inherit protection. Use separate ' +
  ...
  },
  {
    ...
    title: 'ASP.NET Core',
    content:
      'Apply `[Authorize]` via a global filter with `[AllowAnonymous]` as the explicit exception. Use policy-based ' +
  ...
  // ── Cloud and gateways ────────────────────────────────────────────────────
  {
    ...
    content:
      'Enforce authentication with a Lambda authorizer or Cognito authorizer at the gateway, and keep the same check in ' +
  ...
  },
  {
    ...
    content:
      'Use inbound policies — `validate-jwt` for authentication, `rate-limit-by-key` for throttling, and `cors` for ' +
  ...
  },
  {
    ...
    title: 'Google Cloud',
    content:
      'Use API Gateway or Cloud Endpoints with a JWT provider for authentication, and Cloud Armor for rate limiting and ' +
  ...
  },
  {
    ...
    title: 'nginx',
    content:
      'Add security headers with `add_header ... always` so they are present on error responses too. Use `limit_req` ' +
  ...
  // ── Authentication technologies ───────────────────────────────────────────
  {
    ...
    title: 'JWT',
    content:
      'Pin the accepted algorithm explicitly and reject tokens that declare another. Verify `exp`, `nbf`, `iss` and ' +
  ...
  },
  {
    ...
    title: 'OAuth 2 / OIDC',
    content:
      'Use authorization code with PKCE for public clients. Validate `state` to prevent CSRF and register exact redirect ' +
...
export class SecurityKnowledgeRegistry {
  private readonly byKey = new Map<string, SecurityPlaybook[]>();
  ...
  /**
   * Retrieves only the playbooks relevant to one finding.
   *
   * Targeted rather than exhaustive: dumping all nineteen playbooks into every
   * prompt would cost thousands of tokens per finding and bury the two entries
   * that actually apply.
   */
  select(input: {
    ...
    for (const technology of input.technologies ?? []) collect(technology);

    return { playbooks, version: KNOWLEDGE_PACK_VERSION };
  }
}
