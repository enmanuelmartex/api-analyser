/**
 * The in-document paginator.
 *
 * WHY THIS EXISTS
 *
 * A report is a document, not a web page printed at the end. The previous
 * template handed Chromium one continuous column and let the print engine cut
 * it wherever it landed, which produces a scroll with page boundaries through
 * it — no page can have a composition of its own, running furniture can only
 * live in the paper margin, and the cover is just "the top of the scroll".
 *
 * So the document declares `@page { margin: 0 }` and builds its own pages:
 * every sheet is a real 210×297mm box with its own padding, header, footer and
 * page number. Content is measured and distributed into those sheets here,
 * because only the browser knows how tall a block renders.
 *
 * The script is inlined into the document rather than injected by Puppeteer, so
 * the stored HTML snapshot is self-contained: re-printing an archived report
 * years later paginates identically, and opening the HTML export in a browser
 * shows the same pages the PDF has.
 *
 * CONTRACT WITH THE TEMPLATE
 *
 *   #report-source     <template> holding the document in source order.
 *   #report-furniture  <template> holding one `.ph` header and one `.pf` footer,
 *                      cloned into every generated sheet.
 *   #report-pages      where finished sheets are appended.
 *
 * Per-item attributes, all optional:
 *
 *   data-sheet         this element IS a finished sheet (the cover); it is
 *                      placed whole and gets no furniture.
 *   data-start="page"  begin a new sheet before this block.
 *   data-keep-next     glue to the block after it, so a heading is never
 *                      stranded at the foot of a page.
 *   data-split         how the block may be broken when it does not fit an
 *                      empty page: "children" moves trailing child blocks to a
 *                      continuation, "rows" moves trailing table rows under a
 *                      repeated header. Absent means atomic.
 *   data-title         label used on the "(continued)" line of a continuation.
 *   data-anchor        resolves `[data-toc-page="<id>"]` to this block's page.
 *
 * Rows carrying `data-with-prev` are never separated from the row above them.
 *
 * Anything that cannot be split and is genuinely taller than one page is
 * clipped rather than allowed to bleed onto the paper edge. The template caps
 * the only unbounded elements (evidence dumps) so that path stays theoretical.
 */
export const PAGINATOR_SCRIPT = String.raw`
(function () {
  'use strict';
