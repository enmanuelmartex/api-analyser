...
function owaspCoverageFor(globalIndex) {
    const offset = ((globalIndex % 5) - 2) * 3;
    const map = {};
    for (const id of OWASP_IDS)
        map[id] = clamp(Math.round(OWASP_BASE[id] + offset), 5, 100);
    return map;
}
...
async function main() {
    ...
    if (!findingsAssessmentId)
        throw new Error('No assessments to seed — check MONTHLY_SCORES.');
    const issueBySeverity = new Map();
    for (const issue of DEMO_ISSUES)
        if (!issueBySeverity.has(issue.severity))
            issueBySeverity.set(issue.severity, issue);
    ...
    for (const [weekKey, counts] of Object.entries(WEEKLY_FINDINGS)) {
        ...
        for (const [severity, count] of Object.entries(counts)) {
            for (let n = 0; n < (count ?? 0); n += 1) {
                occurrenceSeeds.push({ severity: severity, detectedAt: new Date(now.getTime() - offsetDays * DAY_MS - n * 3600_000) });
            }
        }
    }
    const visibleCount = occurrenceSeeds.length;
    const prevSeverities = ['LOW', 'MEDIUM', 'HIGH', 'INFO', 'MEDIUM', 'LOW'];
    for (let k = 0; k < PREVIOUS_PERIOD_TOTAL; k += 1) {
        const offsetDays = 63 + (k % 6) * 7;
    ...
    const perMonth = assessmentSeeds.reduce((acc, seed) => {
